...
description: Use when working in plugins/obsidian/ — the TypeScript Obsidian plugin that mirrors worklode into a vault. Triggers: "the obsidian plugin", "the vault", "sync notes", "wire types", "writeBack", "conflict note", "pnpm install", "build the plugin", or any change to a json-tagged internal/model shape that plugins/obsidian/src/api/types.ts hand-mirrors. Not for the Go binary or the web cockpit.
---

# The Obsidian mirror

`plugins/obsidian/` is a TypeScript Obsidian plugin, built and shipped
...
Its wire types (`plugins/obsidian/src/api/types.ts`) are hand-kept against
...
pnpm -C plugins/obsidian install --frozen-lockfile
pnpm -C plugins/obsidian test                           # also: typecheck, build
```

CI runs the `obsidian` job only when a PR touches `plugins/obsidian/` or
