 ...
 *  "---". A hand-written document body could in principle do so, but that
 *  reads as a malformed fence here rather than a false negative -- the same
 *  outcome an unclosed fence gets. */
 ...
 *  closed fence whose YAML throws, or parses to null, an array, or a scalar,
 *  is reported `malformed` and the block is stripped from `body` rather than
 *  left to render as a duplicate, unparseable fence. */
...
export async function docToNote(d: Doc): Promise<Note> {
  ...
  if (split.malformed) {
    note.conflict =
      `${d.id}: frontmatter block is not a YAML mapping; ignored, and the note was rendered with no ` +
      `author frontmatter.`;
...
function renderProjectBody(docs: Doc[], tasks: TaskListDetail[]): string {
  const docLines = sortIds(docs.map((d) => d.slug)).map((slug) => `- ${wikilink(slug)}`);
