...
// that anchor and not the rules under it; a whole-document ref names every
// rule the document contains; an entry naming no rule is kept verbatim. A
// plan has no doc_rules rows.
func TestCoversResolveToRules(t *testing.T) {
	...
	if got := coveredRuleNumbers(t, s, mixed.ID); !slices.Equal(got, []int64{1, 3}) {
		t.Errorf("rule ref and section ref cover %v, want [1 3]", got)
	}
