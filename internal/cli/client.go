...
// syncBatchBytes bounds the encoded documents in one /api/v1/docs/sync
// request. The corpus grows with every spec and plan, so the whole of it in
// one body eventually exceeds whatever limit sits in front of the server:
// nginx's default client_max_body_size and the server's own maxAPIBody are
// both 1 MiB. Half of that leaves room for the envelope, for JSON escaping,
// and for a proxy configured below the default.
const syncBatchBytes = 512 << 10

// docSyncNamesShown caps how many document refs a partial-failure message
// spells out before it falls back to a count.
const docSyncNamesShown = 10

// ref names a document the way a user reads it: "spec/25", "plan/25-1". The
// server owns the real id (it prefixes the project key), so this is the only
// name the client can give a document it never managed to send.
func (d DocUpsert) ref() string { return d.Kind + "/" + d.Ordinal }

// DocSyncPartialError reports a doc sync that failed part-way. The corpus
// goes out in several requests (syncBatchBytes), so a failure mid-way leaves
// the documents of earlier batches already stored: Synced counts those, and
// NotSynced names the ones that never left, in corpus order.
type DocSyncPartialError struct {
	Err       error
	Synced    int
	NotSynced []string
}

func (e *DocSyncPartialError) Error() string {
	names, extra := e.NotSynced, ""
	if len(names) > docSyncNamesShown {
		extra = fmt.Sprintf(" and %d more", len(names)-docSyncNamesShown)
		names = names[:docSyncNamesShown]
	}
	return fmt.Sprintf("doc sync failed after %d of %d documents: %v; not synced: %s%s",
		e.Synced, e.Synced+len(e.NotSynced), e.Err, strings.Join(names, ", "), extra)
}

func (e *DocSyncPartialError) Unwrap() error { return e.Err }

// batchDocs splits docs into consecutive batches whose encoded JSON stays
// within budget bytes. The budget is on bytes, not on a document count, so
// one large spec cannot blow it on its own; a document larger than the whole
// budget still goes out, alone, because dropping it would silently lose it
// and whether it is acceptable is the server's call, not the client's. An
// empty corpus yields one empty batch: the request still goes out, as it did
// before batching existed.
func batchDocs(docs []DocUpsert, budget int) ([][]DocUpsert, error) {
	if len(docs) == 0 {
		return [][]DocUpsert{{}}, nil
	}
	var out [][]DocUpsert
	var cur []DocUpsert
	size := 0
	for _, d := range docs {
		...
		if err != nil {
			return nil, fmt.Errorf("encode %s: %w", d.ref(), err)
		}
		if len(cur) > 0 && size+len(b) > budget {
...
// docSyncSender posts one batch. It is a seam so the batching logic can be
// tested without a server.
type docSyncSender func(ctx context.Context, in DocSyncInput) (DocSyncReport, error)

// syncDocsBatched sends in.Docs as several requests bounded by budget bytes
// and merges the reports. It stops at the first failing batch — continuing
// would leave the user guessing which of several failures mattered — and
// returns the report of everything that did land alongside a
// *DocSyncPartialError naming what did not.
func syncDocsBatched(ctx context.Context, in DocSyncInput, budget int, send docSyncSender) (DocSyncReport, error) {
	batches, err := batchDocs(in.Docs, budget)
	if err != nil {
		return DocSyncReport{}, err
	}
	merged := DocSyncReport{DryRun: in.DryRun, Results: []DocSyncResult{}}
	for i, batch := range batches {
		req := in
		req.Docs = batch
		rep, err := send(ctx, req)
		if err != nil {
			var notSynced []string
			for _, rest := range batches[i:] {
				for _, d := range rest {
					notSynced = append(notSynced, d.ref())
				}
			}
			return merged, &DocSyncPartialError{
				Err: err, Synced: len(merged.Results), NotSynced: notSynced}
		}
		if i == 0 {
			merged.DryRun = rep.DryRun
		}
		merged.Added += rep.Added
		merged.Updated += rep.Updated
		merged.Unchanged += rep.Unchanged
...
// (spec 025 §16.2) — as one request per syncBatchBytes of documents. The raw
// bytes it returns are the merged report re-encoded, not a verbatim server
// body, since there is one body per batch.
func (c *Client) SyncDocs(ctx context.Context, in DocSyncInput) (DocSyncReport, []byte, error) {
	rep, err := syncDocsBatched(ctx, in, syncBatchBytes, c.syncDocsBatch)
	if err != nil {
		return rep, nil, err
	}
	raw, err := json.Marshal(rep)
	if err != nil {
		return rep, nil, fmt.Errorf("encode sync report: %w", err)
	}
	return rep, raw, nil
}

// syncDocsBatch posts one batch of documents.
func (c *Client) syncDocsBatch(ctx context.Context, in DocSyncInput) (DocSyncReport, error) {
	body := map[string]any{
		"project":       in.Project,
	...
	if err := json.Unmarshal(raw, &rep); err != nil {
		return DocSyncReport{}, fmt.Errorf("decode sync report: %w", err)
	}
	return rep, nil
}
