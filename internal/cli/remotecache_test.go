...
const testServerURL = "https://wl.example.com"
...
func TestCacheSeparatesServers(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	now := time.Now()

	a := loadCache("https://a.example.com")
	a.putRemote("r", "on-a", now)
	a.putKey("on-a", "AA", now)
	if err := a.save(); err != nil {
		t.Fatalf("save: %v", err)
	}

	b := loadCache("https://b.example.com")
	if project, ok := b.remote("r", now); ok {
		t.Fatalf("other server's remote = %q; want no hit", project)
	}
	if key, ok := b.key("on-a", now); ok {
		t.Fatalf("other server's key = %q; want no hit", key)
	}

	// Writing through one view leaves the other server's section intact.
	b.putRemote("r", "on-b", now)
	if err := b.save(); err != nil {
		t.Fatalf("save: %v", err)
	}
	if project, ok := loadCache("https://a.example.com").remote("r", now); !ok || project != "on-a" {
		t.Fatalf("first server's remote = %q, %v; want on-a, true", project, ok)
	}
}
...
func TestCacheCorruptFileIsEmpty(t *testing.T) {
	...
	if _, ok := loadCache(testServerURL).remote("r", time.Now()); !ok {
