...
// holder (if any) — the `wt task show` view.
...
// ProjectTable prints one row per project: id, name, deploy-gated, repos.
func ProjectTable(w io.Writer, projects []Project) {
	tw := newTabwriter(w)
	fmt.Fprintln(tw, "ID\tNAME\tDEPLOY-GATED\tREPOS")
	for _, p := range projects {
		fmt.Fprintf(tw, "%s\t%s\t%t\t%s\n", p.ID, p.Name, p.DeployGated, strings.Join(p.Repos, ", "))
	}
	tw.Flush()
}
