...
func pages(t *testing.T) map[string]string {
	...
	comps := map[string]templ.Component{
		...
		"doc": Doc(DocView{
			...
			Consolidated:    true,
			ConsolidatedURL: "/docs/WL-SPEC-32",
			SourceURL:       "/docs/WL-SPEC-32?body=source",
			Notes: []DocNoteRow{{
				Anchor:    "sec-10",
				BodyHTML:  template.HTML("<p>" + longTitle + "</p>"),
				CreatedBy: "stig@sunstoneinstitute.ai",
				CreatedAt: now,
				Task:      "WL-716",
			}},
			Reviewers: []DocReviewerRow{
				{Actor: "stig@sunstoneinstitute.ai", Awaiting: true},
				{Actor: "claude-worker-01"},
			},
			Approvals: []ApprovalRow{{
				ID: 9, Kind: "Document", EntityID: "doc:32", Title: longTitle,
				URL: "/docs/32", ProjectID: "worklode", ProjectName: "Worklode backbone",
				Revision: "3", RequiredActorName: "stig@sunstoneinstitute.ai",
...
func TestStylesheetKeepsTheNarrowWidthRules(t *testing.T) {
	...
	for _, c := range []struct{ want, why string }{
		...
		{".dodrow.def{color:var(--ink-3);font-size:14.5px;margin-top:2px;overflow-wrap:anywhere", "a deliverable's artifact address has no soft wrap opportunity in it (WCAG 1.4.10)"},
		...
		{".proj-name{font-size:23px;line-height:1.2;margin:8px010px;min-width:0;overflow-wrap:anywhere", "an unbroken project name must not overrun the 236px sidebar at desktop widths, not just below 880px (WL-90)"},
	} {
