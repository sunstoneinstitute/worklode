...
type RuleView struct {
	Page PageProps
	Rule model.Rule
	// BodyHTML is Rule.Body rendered from markdown and sanitised by
	...
	// CanonicalURL is /projects/<proj>/rule/<n>, the current version's
...
// ruleVersionURL is one rule version's page path, built off the rule's
// own canonical URL the way docVersionURL is built off a document id.
func ruleVersionURL(canonicalURL string, version int) string {
