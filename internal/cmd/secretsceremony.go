...
func runSecretsCeremony(ctx context.Context, cmd *cobra.Command, c *cli.Client, taskID, dir string, declared []string) {
	...
	if len(consentSet) > 0 {
		...
		if !ok {
			...
			// unsatisfied and a later `lode work resume` in a terminal re-runs the
	...
	if _, err := opLookPathFunc(); err != nil {
		fmt.Fprintln(errw, "secrets: 1Password CLI not found — install `op`, sign in, then `lode work resume` to materialize")
		return
	}
	...
	if err := opRunFunc(dir, envFile, taskID, names, declined, errw, errw); err != nil {
		fmt.Fprintf(errw, "secrets: materialization failed: %v — signed in to op? `lode work resume` re-runs the ceremony\n", err)
		return
	}
...
// non-terminal file is an agent-run `lode work next`. Both answer "no" with
// asked=false — a deferral, not a decision, which the caller must not persist
// — and `lode work resume` in a terminal re-runs the ceremony.
func consentToSecrets(cmd *cobra.Command, entries []secrets.Entry) (consented, asked bool) {
	...
	if jsonOut(cmd) || (isFile && !term.IsTerminal(int(f.Fd()))) {
		fmt.Fprintln(errw, "secrets: no operator to ask — deferred, not declined; `lode work resume` in a terminal materializes them")
		return false, false
	}
