...
func TestClaim(t *testing.T) {
	...
	rr := doReq(t, h, "POST", "/api/v1/tasks/WT-1/claim", token, map[string]any{"session_id": "sess-1"})
	if rr.Code != http.StatusOK {
		t.Fatalf("claim status = %d, body %s", rr.Code, rr.Body.String())
	}
	got := decodeMap(t, rr)
	if got["branch"] != "wt/WT-1-fix-the-thing" {
		t.Fatalf("branch = %v, want wt/WT-1-fix-the-thing", got["branch"])
	}
	lease, ok := got["lease"].(map[string]any)
	if !ok {
		t.Fatalf("lease missing: %v", got)
	}
	if lease["task_id"] != "WT-1" || lease["actor_id"] != "alice" || lease["session_id"] != "sess-1" {
...
func TestClaimConflict(t *testing.T) {
	...
	rr = doReq(t, h, "POST", "/api/v1/tasks/WT-1/claim", bobToken, map[string]any{"session_id": "s2"})
...
func TestClaimErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{"blocked task", "/api/v1/tasks/WT-2/claim", 409},
		{"draft task", "/api/v1/tasks/WT-3/claim", 422},
		{"unknown task", "/api/v1/tasks/WT-99/claim", 404},
	}
	for _, tc := range cases {
...
func TestRenewRelease(t *testing.T) {
	...
	rr = doReq(t, h, "POST", "/api/v1/tasks/WT-1/renew", token, map[string]any{"ttl_seconds": 3600})
	if rr.Code != http.StatusOK {
		t.Fatalf("renew status = %d, body %s", rr.Code, rr.Body.String())
	}
	lease := decodeMap(t, rr)
	if lease["task_id"] != "WT-1" || lease["actor_id"] != "alice" {
	...
	bobToken := secondActor(t, st, "bob")
	rr = doReq(t, h, "POST", "/api/v1/tasks/WT-1/renew", bobToken, nil)
	if rr.Code != http.StatusNotFound {
		t.Fatalf("non-holder renew status = %d, want 404", rr.Code)
	}

	rr = doReq(t, h, "POST", "/api/v1/tasks/WT-1/release", token, nil)
	...
	rr = doReq(t, h, "POST", "/api/v1/tasks/WT-1/renew", token, nil)
	if rr.Code != http.StatusNotFound {
...
func TestDone(t *testing.T) {
	...
	rr = doReq(t, h, "POST", "/api/v1/tasks/WT-1/done", token, nil)
	if rr.Code != http.StatusOK {
	...
	if _, err := st.ActiveLease(context.Background(), "WT-1"); !errors.Is(err, store.ErrNotFound) {
...
func TestDoneBadStates(t *testing.T) {
	...
	rr = doReq(t, h, "POST", "/api/v1/tasks/WT-1/claim", token, map[string]any{})
	if rr.Code != http.StatusOK {
	...
	// Unknown task: 404.
	rr = doReq(t, h, "POST", "/api/v1/tasks/WT-99/done", token, nil)
	if rr.Code != http.StatusNotFound {
...
func TestAbandon(t *testing.T) {
	...
	// From ready.
	rr := doReq(t, h, "POST", "/api/v1/tasks/WT-1/abandon", token, nil)
	if rr.Code != http.StatusOK {
	...
	rr = doReq(t, h, "POST", "/api/v1/tasks/WT-2/claim", token, map[string]any{})
	if rr.Code != http.StatusOK {
		t.Fatalf("claim status = %d", rr.Code)
	}
	rr = doReq(t, h, "POST", "/api/v1/tasks/WT-2/abandon", token, nil)
	if rr.Code != http.StatusOK {
	...
	if _, err := st.ActiveLease(context.Background(), "WT-2"); !errors.Is(err, store.ErrNotFound) {
		t.Fatalf("active lease after abandon: err = %v, want ErrNotFound", err)
	}

	// From done (terminal): 422.
	rr = doReq(t, h, "POST", "/api/v1/tasks/WT-3/claim", token, map[string]any{})
	if rr.Code != http.StatusOK {
		t.Fatalf("claim status = %d", rr.Code)
	}
	moveToReview(t, st, "WT-3")
	rr = doReq(t, h, "POST", "/api/v1/tasks/WT-3/done", token, nil)
	if rr.Code != http.StatusOK {
		t.Fatalf("done status = %d", rr.Code)
	}
	rr = doReq(t, h, "POST", "/api/v1/tasks/WT-3/abandon", token, nil)
	if rr.Code != http.StatusUnprocessableEntity {
		t.Fatalf("abandon from done status = %d, want 422; body %s", rr.Code, rr.Body.String())
	}

	// Unknown task: 404.
	rr = doReq(t, h, "POST", "/api/v1/tasks/WT-99/abandon", token, nil)
	if rr.Code != http.StatusNotFound {
