...
func (s *server) taskActivityEvents(w http.ResponseWriter, r *http.Request) {
	...
	// -1 from beginEventStream means the client gave no cursor, which here
	// starts the follow at the newest row.
	ctx, rc, cursor, stop, ok := s.beginEventStream(w, r, "activity event stream", "activity id")
	if !ok {
		return
	}
