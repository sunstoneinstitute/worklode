...
// reconcilerSpecBody mirrors internal/store/clauses_test.go's clauseDocV1
// fixture: sec-1 is clause 1, its child sec-1.1 is clause 2, sec-2 is
// clause 3.
const reconcilerSpecBody = "---\nstatus: draft\n---\n# T\n\nIntro.\n\n## 1. One {#sec-1}\n\nA.\n\n### 1.1 Sub {#sec-1.1}\n\nB.\n\n## 2. Two {#sec-2}\n\nC.\n"

// reconcilerFixture is a store seeded with one project (key WL) and one
// accepted-shape spec carrying three clauses, plus a bare *server holding
...
// governedBy reads a task's governing clause refs and sources.
...
// govern links taskID to clause number in project WL with the given source,
// bypassing the gate — used to set up the "a plan already governs this task"
// case.
func (f *reconcilerFixture) govern(t *testing.T, taskID string, clauseNumber int64, source string) {
	t.Helper()
	if err := f.st.Tx(context.Background(), func(tx *sql.Tx) error {
		id, err := store.ClauseIDByRef(tx, "WL", clauseNumber)
		if err != nil {
			return err
		}
		return store.Govern(tx, taskID, id, source, false)
	}); err != nil {
		t.Fatalf("govern %s by clause %d: %v", taskID, clauseNumber, err)
	}
}
...
func TestSpecReconcilerGovernsAPlanlessTask(t *testing.T) {
	...
	if len(gov) != 1 || gov[0].Clause != "WL-CL-1" || gov[0].Source != "gate" {
		t.Fatalf("governed_by = %+v", gov)
	}

	// Redelivery of the same event is absorbed.
	out, err = f.srv.handleSpecReconcile(ctx, prEvent(1001, task.Branch, "Spec: WL-CL-1\n"))
...
func TestSpecReconcilerLinksOncePerTrailer(t *testing.T) {
	...
	if gov := f.governedBy(t, task.ID); len(gov) != 1 || gov[0].Clause != "WL-CL-1" {
		t.Fatalf("governed_by = %+v", gov)
	}
...
func TestSpecReconcilerLeavesPlannedAndNoneAlone(t *testing.T) {
	...
	out, _ = f.srv.handleSpecReconcile(ctx, prEvent(2003, "feature/no-task", "Spec: WL-CL-1\n"))
	if out != eventbus.OutcomeSuppressed {
		t.Errorf("no task: %v", out)
	}
	out, _ = f.srv.handleSpecReconcile(ctx, prEvent(2004, task.Branch, "Spec: WL-CL-999\n"))
	if out != eventbus.OutcomeSuppressed {
	...
	out, _ = f.srv.handleSpecReconcile(ctx, prEvent(2005, task.Branch, "Spec: WL-CL-2\n"))
	if out != eventbus.OutcomeSuppressed {
...
func TestSpecReconcilerIgnoresUnmappedRepo(t *testing.T) {
	...
	ev := prEvent(6001, task.Branch, "Spec: WL-CL-1\n")
