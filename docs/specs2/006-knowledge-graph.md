...
| Supersession (replaces), whole-document and section-level | `dct:replaces` / `dct:isReplacedBy` | **reuse** — between addressable Section nodes for the partial case; no `wl:supersededSection` annotation predicate is minted (014 §3) |
...
| **Document section** | `wl:Section` (+ `wl:lastRevisedIn`) | **MINT** — the addressable, individually linkable part of a DesignDoc that durable links and partial implementation both need; `wl:status` widens to it (014 §2, §3) |
| **Plan** | `wl:Plan` | **MINT** — an executable document, sibling of `wl:DesignDoc` rather than a subclass: reviewable and accept-gated, but mutable and anchor-free (025 §4) |
...
| Component→section evidence | `wl:implements` | **MINT** — the manifest claim "that code meets this section" (014 §6); the remaining work edge is `wl:produces`, Task→Deliverable (033 §4.2) |
...
| **Project** umbrella (named-graph anchor) | `wl:Project` | **MINT** — the unbounded umbrella over a set of repos (the backbone's `projects` table), not a bounded, goal-oriented grouping; anchors projection named graphs (025 §8) |
...
| Task→Project membership | `wl:inProject` | **MINT** — Task→Project, exactly one, derived from `tasks.project_id`; split from `dct:isPartOf` to keep the Task→Task closure type-homogeneous (025 §8) |
...
# disjointness axiom. One unbounded umbrella — wl:Project — covers what they modelled (025 §8).
...
the moment the `owlrl` pass runs (014 §2).
...
wl:implements a owl:ObjectProperty ;              # component → intent (observed)
    ...
    rdfs:comment "That component's code meets that section: wl:implements is only "
                 ...
                 ".worklode/implements.yaml (014 §6), never declared on a task." .

wl:produces a owl:ObjectProperty ;                # execution → intent
    rdfs:domain wl:Task ; rdfs:range wl:Deliverable ;
    rdfs:comment "This task is what makes that deliverable exist (033 §4.2). Component left that "
...
ADR and Spec subclasses (033 §4). **Task execution-state is NOT `wl:status`** — the task state
...
014 §9 makes Task a `prov:Activity`, PROV declares Activity and Entity disjoint, and
...
whatever the source path in rdf-registry is (014 §1).
...
used only in pinned claims; the canonical IRI above stays version-free (014 §1, §4).
...
same argument 014 §1 makes for the prefix rename.
...
they are derived by `observed/repo-implements` from `.worklode/implements.yaml` (014 §6, 033 §4.2).
...
3. **A stable, documented IRI scheme** for Worklode entities — **scheme agreed, override open**;
   ...
   **Base = `https://worklode.io/ns/`** (decided): the `wl:` ontology stays in rdf-registry but its
   ...
   sources live at `rdf/wl/` while the published base carries no `wl/` segment (014 §1), so this
...
2. ~~Section-level supersession: mint or reuse~~ — **RESOLVED:** reuse `dct:isReplacedBy` between
   addressable `wl:Section` nodes; no annotation predicate is minted (014 §3).
