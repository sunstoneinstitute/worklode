...
| Design rule | `wl:Rule`, subclass of `wl:Section`: the lowest heading unit of a spec or ADR, with its own identity, status and versions (12-spec-refactoring-design-tree.md S8 to S11) | mint |
| Rule to rule | `wl:refines`, `wl:constrains`, `wl:conflictsWith` (symmetric), written by an architect; `wl:references`, derived from the rule text on every version (S12, S26) | mint |
