...
- S22 (Q26, revised by your Q6 reply): links stay on the clause they were minted against. A split is a new version of A plus a new clause B (`wasDerivedFrom` A); a merge is a new version of A that absorbs B, with A `supersedes` B. Tasks on B resolve to A through the edge, tasks on A never move. `supersedes` is many-to-many for the migration from specs, where every old clause is withdrawn with successors.
...
- S55 (S22, S27 applied): every clause on a refactor map's left side becomes `withdrawn`, whether or not it names a successor. A2 and S27 already call for `withdrawn`, and S22's merge writes a `supersedes` edge from the survivor to the absorbed clause, which has no live continuation of its own. The `superseded` clause status stays unused by this increment. The S23 stale trigger fires on `withdrawn` only, so a plan arranging a refactored clause goes stale with no new code needed.
...
- S57 (S22 applied): `supersedes` has one writer. `LinkClauses` refuses a `supersedes` edge, naming `lode clause supersede`, and `UnlinkClauses` refuses to remove a `refactor`-sourced edge; a refactor is undone only by a later refactor. `wasDerivedFrom` is an ordinary manual edge with no such restriction.
- S58 (S3, S22 applied): the refactor's S3 half writes events. S22 keeps every governing link on the clause it was minted against, so a refactor never moves a `task_governed_by` row; what S3 requires, that every change to a task's governance is an event, is met by one `task.governance_superseded` event per task governed by a clause the refactor withdraws, naming the old clause and its successors. Increment 3's plan-closing step (S43) is the other writer of governance events.
- S59 (S24 applied): a refactor map is resolved and applied in one transaction; `--dry-run` resolves and rolls back. Re-running an applied map is a no-op per entry: an old clause already `withdrawn` keeps its status, and an edge that already exists is skipped. A successor that is itself withdrawn, or that also appears on the map's left side, fails the whole map, naming the line.
- S60 (S22 applied): resolution through the supersession chain is read-side only. `TaskGovernance` gains `ResolvesTo`, the refs of the live clauses reached from a withdrawn governing clause by following `supersededBy` edges transitively, empty when the governing clause is live. `lode task show` and `GET /api/v1/tasks/{id}` carry it (03-tasks-and-execution.md §4); there is no cockpit view yet.
