...
- **Declared layer (intent).** Authored *with* a design doc, crit-reviewed while the doc is a
  draft with an open review task (006 §4) and gated on that review's resolution before it counts
...
| `…/graph/observed/repo-implements` | observed | implements-manifest deriver (014 §6) |
...
`.worklode/implements.yaml` and emits `<component> wl:implements <section>` (014 §6):
...
have no v1 source — are specified in 015 §2–§6.
...
Two more standing queries arrive over the implements manifest (014 §6): document coverage, and a
...
`wl:implements` edge (014 §5–§6), not a doc-level status or a Task→DesignDoc join: a Section is
...
```
(`wl:status`'s SKOS scheme — ending at `superseded` — is defined in 006 §4, D4.)
...
| `lode drift --docs` | stale and orphaned implementation claims (014 §6), alongside this command's other drift |
| `lode gaps` | 3.2 doc gaps + unmatched-path coverage gaps |
| `lode specs --drifted` | the stale-claim query above (§3; 014 §6) |
...
coverage (014 §10) — is a sibling surface for the documents themselves rather than for drift
across them.
...
frontier**. Per-section coverage badges and version history join the document view (014 §10).
