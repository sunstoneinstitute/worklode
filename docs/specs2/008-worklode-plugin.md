...
- docs/specs/011-delivery-lifecycle.md
...
## 1. Design lens & CLI naming {#sec-1}
...
## 2. Worktree-bound lease lifecycle {#sec-2}
...
## 3. Branch names are a server-rendered template {#sec-3}
...
### 3.2 Template validation {#sec-3.2}
...
## 6. Naming configuration surface {#sec-6}
...
### 7.1 Migrating an existing worktree {#sec-7.1}
...
### 7.2 PR and push correlation across the cutover {#sec-7.2}
...
## 8. Naming and guard tests {#sec-8}
...
- **Governing design** — the governing **Spec section**, a `wl:Section` node reached via
  `wl:governs` (006) and bounded by construction (014 §3); not a Spec or Plan excerpt, and never
  the whole doc.
...
| `/lode-spec` | Enter graduated design authoring (D15) — produce only the artifacts the task warrants, drawn from {ADR, Spec, task subtree} and never a Plan document (014 §2); see *authoring-design-as-graph*. |
...
## 13. Skills (judgment only) {#sec-13}
...
- **`authoring-design-as-graph`** — graduated to task complexity (D15). There is no Plan artifact:
  ...
  (014 §2) — do not ceremony-tax small tasks. Get **crit** review; write the **declared-layer**
...
## 16. The harness landscape (August 2026) {#sec-16}
...
### 16.1 `SKILL.md` became a standard {#sec-16.1}
...
### 16.2 Skill and instruction delivery {#sec-16.2}
...
### 16.4 Observability and UI surfaces {#sec-16.4}
...
## 17. Multi-harness design {#sec-17}
...
### 17.1 The harness adapter {#sec-17.1}
...
### 17.3 Skill delivery — one store, many doorways {#sec-17.3}
...
### 17.6 Telemetry ingest (v2) {#sec-17.6}
...
10. `lode task brief <id> --json` returns one bounded payload (task + concern/priority + governing
    ...
    without reading repo files; there is no Plan excerpt to assemble (014 §2).
