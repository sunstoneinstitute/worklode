...
**Governing rules.** A task carries direct links to the design rules that govern it (05-documents.md §4, 12-spec-refactoring-design-tree.md S2 and S3), stored in `task_governed_by (task_id, rule_id, rule_version, source)`. Accepting a plan links every task it mints to each rule the plan's `covers` edges reach: a section-scoped edge reaches the rule at that anchor and the rules arranged under it, a document-scoped edge reaches every rule the document arranges. The plan is never the source of truth for the link afterwards; a stale or withdrawn plan leaves its tasks governed. `rule_version` records the rule version current when the link was made, and the link resolves to the rule's newest version. A link may be pinned when it is made (`--pin`); a pinned link resolves to that version and reports the versioned page `/projects/<proj>/rule/<n>/<ver>`. Governing the same rule again without the pin unpins it. After minting, only an architect changes the links, by hand, and every change is an event on the task (`task.governed`, `task.ungoverned`). A link's `source` is `plan`, `manual` or `gate`: the plan that minted the task, an architect's hand, or the design authority gate reading a `Spec:` trailer on a planless task's pull request (11-design-authority-gate.md §4). The gate writes only when no plan link exists and never removes a link. A task created without a plan may name its rules at creation and may acquire them later. A refactor (`lode rule supersede`, 12-spec-refactoring-design-tree.md S24) never moves a task's link: a task governed by a rule the refactor withdraws keeps that link, and it reads the live rules the withdrawn rule resolves to by following `supersededBy` edges (S22). Each task the refactor touches this way gets one `task.governance_superseded` event naming the old rule and its successors; the link rows themselves are untouched (S3, S58). Those events live in the event log only; task timelines show state transitions and leave governance changes to the task detail and `lode event tail` (12-spec-refactoring-design-tree.md S34). Re-accepting a plan whose `covers` widened governs only the tasks that re-accept mints; earlier tasks keep their links until an architect adds to them (S31). A rule ref carries its project key, so a task may be governed by another project's rule (S32). When a plan closes, by `lode doc withdraw` or by its last task closing, every task minted from it that carries no governing link is linked to the plan's arranged rules before the status moves, so a closed plan's tasks each carry at least one link (12 S5, S43).

| Surface | Effect |
|---|---|
| `POST /api/v1/tasks` with `governed_by: [WL-RULE-12]` | create with links |
| `POST` / `DELETE /api/v1/tasks/{id}/governed-by` with `{"rule": "WL-RULE-12", "pin": true}` (`pin` is read only by `POST`; `DELETE` ignores it) | add or remove one link |
| `GET /api/v1/tasks/{id}` field `governed_by` | the links with each rule's current version; a withdrawn governing rule also carries `resolves_to`, its live successors (S60) |
| `lode task add --governed-by WL-RULE-12` | create with a link, repeatable |
| `lode task govern <id> --by WL-RULE-12` / `ungovern` | add or remove one link |
| `lode show <id>` | one `governed by` line per link, noting when the rule has a newer version, with a `->` line of live successors when the governing rule is withdrawn |
...
It does one thing. On a GitHub `pull_request.*` or `push` event, find the task from the branch name, or, when the branch does not resolve to a task, from a `Worklode-Task:` trailer in the pull request body; then read the `Spec:` trailer from the pull request body or the pushed commits, newest first. When it names a rule (`WL-RULE-<n>`) or a section (`WL-SPEC-4 sec-5`, resolved to the rule arranged at that anchor) and the task carries no `source = 'plan'` link, write a `governedBy` link with `source = 'gate'`. `none`, a missing or malformed trailer, an unknown task, an unknown target, a task already governed by a plan and an already gate-governed task are counted and change nothing. The `task.governed` event's external id, `spec-reconciler-<task id>-<declaration>`, names the link the trailer asks for, so redelivery and every later push carrying the same trailer for the same task collide with the first one and change nothing. The subscriber runs behind `SubscriberLock` beside `doc-lifecycle` and starts with it. The subscriber starts at the beginning of the event log, so the release that introduces it replays the recorded history once, and almost every replayed event is counted and changes nothing.
