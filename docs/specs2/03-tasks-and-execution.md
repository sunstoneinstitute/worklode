...
| `GET /api/v1/tasks/{id}` field `governed_by` | the links with each clause's current version; a withdrawn governing clause also carries `resolves_to`, its live successors (S60) |
...
| `lode show <id>` | one `governed by` line per link, noting when the clause has a newer version, with a `->` line of live successors when the governing clause is withdrawn |
