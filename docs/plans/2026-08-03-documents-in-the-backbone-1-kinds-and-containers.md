...
covers:
  ...
  - docs/specs/025-documents-in-the-backbone.md#sec-13
  - docs/specs/025-documents-in-the-backbone.md#sec-17
---
...
**Goal:** Implement 025 §10, §8, §9 and the 018-narrowing half of §5: task
...
**Read first:**
- 025 §9.2 ("What this leaves of 018"), §6, §8, §13 AC1/AC3
...
**Non-goals:** the document store (parts 2–3); anything in 025 §22; graph
projection of `wl:inProject` (006 §11 owns projection, and no projector is
...
- Kind enum today: migration `0009` CHECK is
  ...
  carry the same seven. Target (025 §10):
...
- Child-acquisition paths today: `Decompose`, `POST /tasks/{id}/edges`
  ...
  `lode task add --parent`, `lode task parent --under`. Target (025 §24 AC3):
...
//
// ns/ owns the shared schema (025 §17); a change here is a change to the
...
The rename half of 025 §9.2: every 018 guard survives, applied to a task that
...
// is the container predicate of 025 §9.2: a task with children is never
...
- `hierarchy_resolve.go` `ResolveHierarchy`: delete the `kind != kindEpic`
  ...
  *parent*/*container*, citing 025 §9.2.
...
- Update the doc comment: decompose is the **only** path by which a task
  acquires children (025 §24 AC3), and re-running it on a task with children
...
Everything in this task lands in **one commit**, per 025 §10 — the agreement
...
-- re-kind any surviving epic rows to 'chore'. 025 §10 expects no epic rows;
...
`wlc:spec` to `wlc:design` with 025 §10's widened definition:
...
wlc:design a skos:Concept ; skos:inScheme wlc:TaskKind ; skos:prefLabel "design" ;
    skos:definition """Author or revise a Worklode document — spec, ADR, or plan; the document
        produced is reachable via prov:wasGeneratedBy (025 §12). Closes when the document is
        accepted; never an umbrella held open against coverage (025 §10).""" .
```

Update the scheme's header comment (the constraint mirror is now 025 §10, and
...
Pure `ns/` state change; 025 §13 gives the target Turtle verbatim and §4/§3
...
- Replace `wl:Project` (lines 124-127) and add `wl:inProject` with 025 §13's
...
- Add `wl:Plan` beside the `wl:DesignDoc` subclasses (after line 66) with
  025 §9's Turtle verbatim (sibling of DesignDoc, `wl:layer wlc:execution`,
  ...
  (026 §5.2). Update the trailing "wl:Plan dropped (025 §2)" comment
  (line ~450) to record the 025 §9 return.

- [ ] **Step 2: `ns/shapes.ttl` and `ns/concept.ttl`**

- Task shape (lines 170-173): `wl:inWorkstream` minCount-1 becomes
  `wl:inProject` `sh:minCount 1 ; sh:maxCount 1`, message citing 025 §13.
- `ns/concept.ttl`: delete `wlc:proposed` (lines 30-31) and drop it from
  ...
  with an open review task (025 §7). Update the section comment.
