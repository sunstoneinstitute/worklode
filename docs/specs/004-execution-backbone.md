...
to check out, so `lode worktree next` must never hand an agent one. Decomposition work
...
to detect it by. It keeps a decision out of `lode worktree next`'s candidate set and out
