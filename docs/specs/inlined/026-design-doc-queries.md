...
# Spec 026 — Design-doc queries and plan coverage — consolidated view
...
> Source of record: [`docs/specs/026-design-doc-queries.md`](../026-design-doc-queries.md) — status **draft**.
>
...
### 3.1 A claim takes effect when its author is accepted
...
### 3.2 Consolidation is a fixpoint, not a depth-bounded walk
...
## 4. Reference resolution and integrity
...
### 4.1 Anchor permanence, enforced at commit time
...
### 4.2 Shorthand references resolve in three tiers
...
### 4.3 `NO-SPEC` means "no governing spec"
...
## 5. A plan covers sections; it implements none
...
### 5.2 Plans carry `status` and `task`
...
### 5.3 `defers` hands a section to a named owner
...
### 6.2 `wl:implements` splits, and states its domain
...
### 6.3 The qualified node names both its ends
...
### 7.1 Documents the rename reaches
