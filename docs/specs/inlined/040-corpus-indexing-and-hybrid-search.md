...
# Spec 040 — Corpus indexing and hybrid search — consolidated view
...
> Source of record: [`docs/specs/040-corpus-indexing-and-hybrid-search.md`](../040-corpus-indexing-and-hybrid-search.md) — status **draft**.
>
...
### 2.1 Anthropic has no embeddings API
...
### 2.2 The width is the contract, not the model
...
## 3. Provider interface: queries and documents are not the same input
...
### 4.2 Documents chunk on their own section boundaries
...
### 4.3 Every chunk carries its own context header
...
### 4.4 Tasks are one chunk until they are not
...
### 6.2 Lexical arm: `simple`, and why that matters
...
### 6.3 Fusion: reciprocal rank, k = 60
