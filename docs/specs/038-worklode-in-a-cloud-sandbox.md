...
### 3.3 Egress is default-deny, with a named baseline {#sec-3.3}
...
- **Minting.** `POST /api/v1/tasks/{id}/tokens` mints an ordinary `wl_` token whose row is bound
  ...
  job with it, nothing persists). Default expiry is the lease TTL (time-to-live).
...
instruction, both held at 100% across 280 repeats. It stays a followed instruction, not an enforced
guarantee.

**Server-to-session delivery works.** `docs/plans/2026-08-25-steering-instructions.md` shipped
`lode channel serve`: a local stdio MCP (Model Context Protocol) relay the session spawns as a
child process, which polls
...
### 5.1 Why not an external agent runtime, yet {#sec-5.1}
