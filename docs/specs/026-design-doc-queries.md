...
change what a plan *is* (025 §9), how acceptance mints tasks (025 §9.2), or how
...
(025 §11), is **already section-scoped** — its entries key on `section:` with a pinned
...
explaining. The plans were backfilled instead — a **spent** plan is `superseded` (025 §9: a
...
025 §14.3 adds `<PROJECTKEY>-<TYPE>-<n>` alongside §4's path forms. Which tier applies is
...
025 §14.3 has `<TYPE>` checked against the target's kind, and no document declares one until
...
has none. Committing the key to the repo is safe because 004 §2.1 makes it immutable, and it is
...
- **Written `NO-SPEC`, not `<KEY>-SPEC-0`.** The keyed form is recognised and reported so it
  ...
  (025 §14.3) — though a *foreign* key such as `rdf-registry:ADR-0006` pads by its own convention,
...
025 §9's ruling stands: a plan is a document, takes no anchors, and names spec
...
tasks its acceptance mints (025 §9.2), which produce the deliverables the
...
**A bare string keeps working and means `full`.** `covers: docs/specs/004-execution-backbone.md#sec-5.2`
...
### 5.4 Decomposition is complete, or it is not {#sec-5.4}
...
> graph-live deliverable. · The worklode component **implements** spec 006 §9.
...
`ns/` is the schema source (025 §17). The terms below are mirrored there, and
...
**`wl:covers` was taken.** 006 §11.1 used it for the delivery frontier —
...
"deliberately absent"; 025 §9 restored it as a sibling of `wl:DesignDoc` and
`ns/` never caught up. `wl:covers` needs it as a domain, so the class is minted
here per 025 §9's Turtle and joins the top-level disjointness axiom.
...
relations at once — `Component → Section`, the manifest claim 025 §11 added, and
`Task | Issue | PullRequest → Deliverable | Component`, 006 §3's original — and
...
Both now state a domain, so SHACL can constrain them and 025 §11's coverage
...
projector 025 §5 anticipates. 006 §11.1 declined to mint a per-environment frontier
...
025 §14's key table row is corrected in place: it listed `implements` as a
...
- the references section gains the four reference forms of §4, 025 §14.3's shorthand with the
...
- **Forcing a migration outside this tree** — the rename's supporting commit normalised
  ...
  not rewrite plans.
- **Implementation coverage** — 025 §11's component claim is untouched, and keeps
  `wl:implements`.
- **The backbone `docs` tables** and the graph projection — 025 owns both, and this spec is
  ...
  declares; no new contract (006 §11).
...
- **A `doc revise` practice note.** When a document's consolidation is mostly borrowed text,
  that is the signal to write a superseding revision (025 §7) — and the consolidation is what
