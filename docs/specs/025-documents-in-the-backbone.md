...
amends:
  "#sec-10":
    ...
    - 029-research-work-in-the-backbone.md#sec-2
  "#sec-15":
...
### 4.5 Store-level version history {#sec-4.5}
...
### 6.2 Rule 2 is detected, not refused {#sec-6.2}
...
`lode doc transfer` (named documents, or every document one actor owns with `--from`), and the
transfer emits `doc.owner_changed` (§15.2).
...
### 10.1 A decision's data {#sec-10.1}
