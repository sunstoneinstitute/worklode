...
- **Most automatic syncs are incremental, and an incremental sync is
  ...
  different backbone.
- **A doc note's text costs one extra request, spent only when it changed.**
  `GET /api/v1/docs` blanks the markdown on every row — a corpus listing would
  otherwise be the largest response the server sends — so a document's text
  comes from `GET /api/v1/docs/{id}`, one request per document. A full sync
