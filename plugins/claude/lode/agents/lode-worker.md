...
`lode worktree next --json`.

1. Run the selection rule. Nothing ready: stop and report "no ready work".
2. Claim it (`lode worktree next [id] --json`) and `cd` into the worktree the
...
4. Verify the report against the brief's definition of done — including that
   ...
   open its PR *before* `lode worktree done --json`. `done` submits the task
   ...
   `lode worktree block --on <id> --json`. Back to 1.
