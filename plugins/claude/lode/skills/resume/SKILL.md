...
Run `lode work resume --json`, adding only a worktree directory if those arguments
name one. `lode work resume` takes at most one positional argument, so anything else
