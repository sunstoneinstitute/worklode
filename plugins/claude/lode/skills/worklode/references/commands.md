...
Shortcuts: `lode board` runs `lode task board`; `lode next` runs `lode work next`; `lode overview` runs `lode project overview`; `lode status` runs `lode work status`.
...
- `lode project overview` — One-screen roll-up: drift counts, gaps, frontier, critical head
  Flags: --project, --repo
- `lode project rally` — Show a project's open rally and its transitive open blockers
